...
def np_to_torch_dataloader(x_in,target,batch = 1,Params=None):
    '''
    ...
    if Params is not None:
        ...
        param_tensor = torch.from_numpy(Params)
        target_tensor = torch.from_numpy(target)
        tensor_data = torch.utils.data.TensorDataset(x_in_tensor,param_tensor,target_tensor)
        ...
        x_in_tensor = torch.from_numpy(x_in)
        target_tensor = torch.from_numpy(target)
        tensor_data = torch.utils.data.TensorDataset(x_in_tensor,target_tensor)
        data_loader = torch.utils.data.DataLoader(tensor_data,batch_size = batch,shuffle = True,drop_last=True)
        return data_loader