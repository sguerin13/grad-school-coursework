...
class Decoder(nn.Module):
    def __init__(self):
        ...
        # 1st block - 4 x 8
        self.dconv11  = nn.ConvTranspose2d(256*5,256,2,stride = 2)
    ...
    def forward(self,x):#,x,max_ind_1,max_ind_2):
        ...
        x = F.relu((self.dconv13(x)))
        
        x = F.relu((self.dconv21(x)))
        x = F.relu((self.dconv22(x)))
        x = F.relu((self.dconv23(x)))

        x = F.relu((self.dconv31(x)))
        x = F.relu((self.dconv32(x)))
        x = F.relu((self.dconv33(x)))

        x = F.relu((self.dconv41(x)))
        x = F.relu((self.dconv42(x)))
        x = F.relu((self.dconv43(x)))

        x = F.relu((self.dconv51(x)))
        x = F.relu((self.dconv52(x)))
...
class EncoderDecoder(nn.Module):
    ...
    def forward(self,map,params):
        ...
        # and reshape output
        # example input 15 x 5 x 3 x 64 x 128
        map_shape = map.shape

        # 15*5 x 3 x 64 x 128
        map = map.view(map.shape[0]*map.shape[1],map.shape[2],map.shape[3],map.shape[4])
        # 15*5 x 3
        params = params.view(params.shape[0]*params.shape[1],params.shape[2])

        # 15*5 x 256 x 2 x 4
        x_enc = self.encoder(map,params)
        # 15 x 5 x 256 x 2 x 4
        x_enc = x_enc.view(map_shape[0],map_shape[1],x_enc.shape[2],x_enc.shape[3],x_enc.shape[4])

        # now we stack it along the channel dimension
        x_enc = x_enc.view(map.shape[0],map_shape[1]*x_enc.shape[2],x_enc.shape[3],x_enc.shape[4])
